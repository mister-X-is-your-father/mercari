...
lady_grandchild_array = [
  ...
  ['ヘアゴム/シュシュ','ヘアバンド/カチューシャ','ヘアピン','その<e4><bb><96>'], 
...
toy_grandchild_array = [
  ...
  ['武具','切手/官製はがき','貨幣/金貨/銀貨/記念硬貨','印刷物','ノベルティグッズ','その他'],
  ['トイガン','個人装備','その他'],
...
cosmetic_grandchild_array = [
  ...
  ['エッセンシャルオイル','芳香器','お香/香炉','キャンドル','その他'],
  ['ダイエット食品','エクササイズ用品','体重計','体脂肪計','その他'],
  ['健康用品','看護/介護用品','救急/衛生用品','その他']
]

parent = Category.create(name: 'コスメ・香水・美容')
cosmetic_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
  cosmetic_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

electric_child_array = [
  'スマートフォン/携帯電話',
  'スマホアクセサリー',
  'PC/タブレット',
  'カメラ',
...
electric_grandchild_array = [
  ...
  ['ヘアドライヤー','ヘアアイロン','美容機器','マッサージ機','電気シェーバー','電動歯ブラシ','その他'],
