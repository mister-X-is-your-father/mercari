$(function(){
  ...
  function buildCategory(category){
    let html = `<option value="${category.name}">${category.name}</option>`
    return html
  };
  ...
  function appendChildCategory(insertHTML){
    let ChildCategoryHTML = 
      `<div class="select-wrap iu-child">
        <select id="parent-categories" name="item[category_id]">
          <option value="">---</option>
          ${insertHTML}
      </div>`
  ...
  $('#parent-categories').on('change', function(e){
    ...
    if (parent_name != ''){
      $.ajax({
        ...
        type:        'GET',
        data:        { parent_category: parent_name },
        dataType:   'json'
      })
      .done(function(child){
  ...
  $('.select-wraps').on('change', '.select-wrap.iu-child', function(e){
    e.preventDefault();
    let child_name = $(this).children().val();
    if (child_name != ''){
      $.ajax({
        url:         '/items/get_grandchild_categories',
        type:        'GET',
        data:        { child_category: child_name },
        dataType:   'json'
      })
